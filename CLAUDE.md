...
- **`generator/`** — the `pgbase` bin: Prisma DMMF → a static TS schema module. It exists because
  logical decoding reports physical names only, so the Prisma↔physical mapping must survive as
  runtime *data*; nothing Prisma generates carries it. `SCHEMA_FORMAT_VERSION` lives in
  `src/version.ts` and must be bumped when the emitted shape changes.

### Invariants that are easy to break

- **`src/index.ts` is intentionally empty.** Everything ships as a subpath. Adding a module means
  adding it to *both* `tsup.config.ts`'s `entry` array and `package.json`'s `exports` map, with
...
- **Error mapping:** `ReadValidationError` → 400 and `ScopeViolationError` → 403 in pgbase's own
  filter; `ScopedRowNotFoundError` is left to the consuming app, and is deliberately identical to
  "row does not exist" because distinguishing the two is itself a disclosure.
...
The codebase's comments are dense *rationale*, not description — they explain the failure mode a
